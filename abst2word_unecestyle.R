...
render_function <- function(abstract_title, # These function arguments match with the parametres in the RMarkdown file
                            ...
                            coauthors)
{
  rmarkdown::render("abst2word_unecestyle.Rmd", # This is the RMarkdown file
                    ...
                    output_dir = "docs/DissComm2023", # This is where the MS Word documents are output
                    # For now, I've added a sequential no to the output title to handle multiple submissions by the same authors
                    output_file=paste0("No",i,"_Abstract", " - ", first_name, " ", last_name, " - ", format(Sys.Date(), format="%Y_%m_%d"))
